...
import { DIFFICULTY_OPTIONS, GAME_SUBTITLE, GAME_TITLE } from "@shared/game";
    ...
    <div className="mx-auto grid min-h-screen max-w-7xl gap-6 px-4 py-5 md:px-6 lg:grid-cols-[1.15fr_0.85fr] lg:py-8">
      <section className="panel-strong relative overflow-hidden p-6 md:p-8">
        <div className="absolute inset-0 opacity-60">
          <div className="absolute -left-16 top-10 h-40 w-40 rounded-full bg-gold/10 blur-3xl" />
          <div className="absolute right-0 top-0 h-52 w-52 rounded-full bg-cyan-300/10 blur-3xl" />
          <div className="absolute bottom-8 left-1/3 h-44 w-44 rounded-full bg-emerald-300/10 blur-3xl" />
        </div>
        ...
        <div className="relative">
          ...
          <div className="mt-6 flex flex-wrap gap-3 text-xs uppercase tracking-[0.25em] text-mist/65">
            <span className="rounded-full border border-white/10 bg-white/5 px-4 py-2">Multijugador</span>
            <span className="rounded-full border border-white/10 bg-white/5 px-4 py-2">Mision arcade</span>
            <span className="rounded-full border border-white/10 bg-white/5 px-4 py-2">En vivo</span>
          </div>
        </div>
            ...
            <button
              ...
              className={`rounded-3xl border p-5 text-left transition ${
                option.id === difficulty
                  ? "border-gold/50 bg-gold/12 shadow-glow"
                  : "border-white/10 bg-white/5 hover:border-white/20 hover:bg-white/10"
              }`}
            >
              <p className="text-xs uppercase tracking-[0.25em] text-gold/70">{option.badge}</p>
              <p className="mt-2 font-display text-3xl text-parchment">{option.title}</p>
      ...
      <section className="flex flex-col gap-6">
          ...
          <div className="mt-4 rounded-2xl border border-gold/25 bg-gold/10 p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="rounded-full border border-gold/30 px-3 py-1 text-xs uppercase tracking-[0.25em] text-gold">
                {selectedDifficulty.badge}
              </span>
            </div>
            <p className="mt-2 text-sm text-mist/80">{selectedDifficulty.summary}</p>
          </div>
        ...
        <form onSubmit={handleJoin} className="panel p-6">
          ...
          <h2 className="mt-3 font-display text-3xl text-parchment">Entrada rápida</h2>
