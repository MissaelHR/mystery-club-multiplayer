    ...
    <div className="mx-auto grid min-h-screen max-w-7xl gap-8 px-4 py-6 md:px-6 lg:grid-cols-[1.2fr_0.8fr] lg:py-10">
      <section className="flex flex-col justify-between rounded-[2rem] border border-gold/20 bg-slate-950/60 p-6 shadow-glow backdrop-blur-xl md:p-8">
        <div className="space-y-5">
          ...
          <h1 className="max-w-3xl font-display text-5xl leading-tight text-parchment md:text-7xl">{GAME_TITLE}</h1>
          <p className="max-w-2xl text-base leading-7 text-mist/80 md:text-xl md:leading-9">{GAME_SUBTITLE}</p>
        </div>
          ...
          {DIFFICULTY_OPTIONS.map((option) => (
            <button
              ...
              className={`rounded-3xl border p-5 text-left transition ${
                option.id === difficulty ? "border-gold/50 bg-gold/10" : "border-white/10 bg-white/5 hover:bg-white/10"
              }`}
            >
      ...
      <section className="flex flex-col gap-6">
        <form onSubmit={handleCreate} className="panel p-6">
          <p className="text-sm uppercase tracking-[0.35em] text-gold/70">Crear partida</p>
          <h2 className="mt-3 font-display text-3xl text-parchment">Host</h2>
        ...
        <form onSubmit={handleJoin} className="panel p-6">
          <p className="text-sm uppercase tracking-[0.35em] text-gold/70">Entrar</p>
          <h2 className="mt-3 font-display text-3xl text-parchment">Jugador</h2>
