  ...
  return (
    <aside className="panel h-fit p-5">
      <div className="flex items-center justify-between">
        <h3 className="font-display text-2xl text-parchment">Resultados en vivo</h3>
        <span className="rounded-full bg-white/10 px-3 py-1 text-xs uppercase tracking-[0.25em] text-gold/80">
          {isPlaying ? "En partida" : "Sala"}
        </span>
      </div>
          ...
          <div key={player.id} className="rounded-2xl border border-white/10 bg-black/15 px-4 py-3">
              ...
              <div>
                ...
                <p className="font-semibold text-parchment">
                  ...
                  {"isHost" in player && player.isHost ? (
                    <span className="ml-2 text-xs uppercase tracking-[0.2em] text-gold/75">Host</span>
                  ) : null}
                </p>
              </div>
            ...
            {"answeredCurrentStage" in player && isPlaying ? (
              <p className="mt-2 text-sm text-mist/75">
                {player.answeredCurrentStage ? "Ya respondió esta etapa" : "Pendiente de responder"}
              </p>
            ) : null}
          </div>
        ))}
